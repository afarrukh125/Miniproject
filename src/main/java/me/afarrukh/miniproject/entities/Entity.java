...
public abstract class Entity {
    ...
    protected float x,
            y; // Making it float achieves smooth movement in our game. Furthermore, the calculations we do in our game
    // will not give whole numbers.
    protected int width, height; // This is the size of the entity
    protected int health;
    private boolean active = true;
    protected final Rectangle
            hitbox; // The interactable boundaries of which the player can detect if a terrain (tile) is interactable or
    // not
    ...
    protected boolean checkEntityCollisions(float xOffset, float yOffset) {
        for (Entity e : manager.getMap().getEntityManager().getEntities()) {
            ...
            if (e.getCollisionBounds(0f, 0f).intersects(getCollisionBounds(xOffset, yOffset)))
                return true; // If the rectangles of the entity's hitbox intersect with the collisionbounds then there
            // is a collision
        }
        return false;
    }
    ...
    public Rectangle getCollisionBounds(float xOffset, float yOffset) {
        return new Rectangle(
                (int) (x + hitbox.x + xOffset), (int) (y + hitbox.y + yOffset), hitbox.width, hitbox.height);
    }
