...
public class EntityManager {
    ...
    private final Comparator<Entity> renderSorter =
            new Comparator<
                    Entity>() { // Sorting entities so they are rendered such that the entities higher up render later
                // than entities lower down

                @Override
                public int compare(Entity a, Entity b) {
                    // Returns -1 if a should be rendered before be or +1 if vice versa
                    if (a.getY() + a.getHeight()
                            < b.getY() + b.getHeight()) // Add the heights so we are checking bottom areas of entities
                    return -1;
                    return 1;
                }
            };
    ...
    public void tick() {
        ...
        while (iter.hasNext()) {
            ...
            e.tick();
            if (!e.isActive())
                iter.remove(); // Removes the entity. This would be the same as e.remove for an individual entity
            // removal
        }
