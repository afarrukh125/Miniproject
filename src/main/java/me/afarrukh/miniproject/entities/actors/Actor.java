...
public abstract class Actor extends Entity {
    ...
    private void moveX() {
        if (xMove > 0) { // Right side with collision detection
            int tx = (int) (x + xMove + hitbox.x + hitbox.width)
                    / Tile.TILEWIDTH; // Dividing by tile width to convert between pixels and tile units
    ...
    private void moveY() {
        if (yMove < 0) { // Up side with collision detection
            ...
            if (!collisionWithTile((int) (x + hitbox.x) / Tile.TILEWIDTH, ty)
                    && !collisionWithTile(
                            (int) (x + hitbox.x + hitbox.width) / Tile.TILEWIDTH, ty)) { // Upper left hand of hitbox
                y += yMove;
            } else {
            ...
            int ty = (int) (y + yMove + hitbox.y + hitbox.height)
                    / Tile.TILEHEIGHT; // For the lower portion of the hitbox, we add the hitbox height
            ...
            if (!collisionWithTile((int) (x + hitbox.x) / Tile.TILEWIDTH, ty)
                ...
                y = ty * Tile.TILEHEIGHT
                        - hitbox.y
                        - hitbox.height
                        - 1; // Subtracting 1 to remove sliding problem (also exists in moving right)
            }
        }
    }
