...
public class Player extends Actor {
    ...
    private int
            charLuck; // If they are a playable character, (at the very least) their attack accuracy has a luck factor
    ...
    private void checkAttacks() {
        ...
        for (Entity e : manager.getMap().getEntityManager().getEntities()) {
            ...
            if (e.getCollisionBounds(0, 0).intersects(attackRectangle)) {
                ...
                return; // We only want to hit one entity at a time. It will hit the first entity in the list that is
                // nearest to it and return
            }
        }
    }
    ...
    public void render(Graphics g) {
        ...
        g.drawImage(
                ...
                (int) (y - manager.getGameCamera().getyOffSet()),
                width,
                height,
                null);
    ...
    private BufferedImage getCurrentAnimationFrame() {
        if (xMove < 0) {
            ...
            if (manager.getKeyManager().atkLeft) return animAtkLeft.getCurrentFrame();
            else return animAtkRight.getCurrentFrame();
        } else {
