...
public class RockPile extends StillEntity {
    ...
    public void die() {
        ...
        int gemChance = Constants.GEM_CHANCE
                + manager.getMap().getEntityManager().getPlayer().getCharLuck();
        if (rng < gemChance) manager.getMap().getItemManager().addItem(Item.gemItem.createCopy((int) x, (int) y));
        else manager.getMap().getItemManager().addItem(Item.rockItem.createCopy((int) x, (int) y));
