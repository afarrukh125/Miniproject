...
public class Map {
    ...
    private int width,
            height; // In terms of tiles this is the size of the map. Note that this is NOT in terms of pixels
    ...
    public void tick() {
        int xStart = (int) Math.max(
                0,
                manager.getGameCamera().getxOffSet()
                        / Tile.TILEWIDTH); // We don't want to render tiles with negative coordinates
        int xEnd =
                (int) Math.min(width, (manager.getGameCamera().getxOffSet() + manager.getWidth()) / Tile.TILEWIDTH + 1);
        int yStart = (int) Math.max(0, manager.getGameCamera().getyOffSet() / Tile.TILEHEIGHT);
        int yEnd = (int)
                Math.min(height, (manager.getGameCamera().getyOffSet() + manager.getHeight()) / Tile.TILEHEIGHT + 1);
    ...
    public void render(Graphics g) {
        ...
        // Using math.max with 0 as one of the parameters ensures we always get a positive number, or math.min ensures
        // we get far right
        int xStart = (int) Math.max(
                0,
                manager.getGameCamera().getxOffSet()
                ...
                getTile(x, y)
                        .render(
                                ...
                                (int) (y * Tile.TILEHEIGHT
                                        - manager.getGameCamera().getyOffSet()));
                // We must use the TILE.TILEWIDTH/HEIGHT to match the units, otherwise only pixels are rendered
            }
        }
    ...
    private void loadMap(InputStream mapPath) {
        ...
        String[] tokens =
                file.split("\\s+"); // Splits the file into string array elements if it is empty space or new line
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                tiles[x][y] = UtilityTasks.parseInt(
                        tokens[(x + y * width) + 4]); // Adding 4 to account for first few elements
            }
        }
    }
